...
- **Mathlib upstreaming candidates.** Several general-purpose lemmas built while closing out `langlands/` sorries look like they'd fill real Mathlib gaps rather than being langlands-specific: cyclic-subgroup-by-divisor classification, ℤ-subgroup-by-index classification, `ProfiniteGrp` universe-bridging via `ULift`, algebraicity-of-completions-under-finite-extension, a `RankOne` instance for bare adic completions, and the discriminant-unit/monogenic-order lemma above. Not scoped as tasks — worth a pass to check which are genuinely missing upstream and PR-worthy.
- **`WeilGroup.lean` polish (adjacent, non-blocking):** continuity of `toArt`, and the topological splitting `W_K ≅ I_K ⋊ ℤ` — flagged but not attempted.
- **Phase 2 (local CFT / Artin map), per `langlands/ROADMAP.md`:** Phase 0 and Phase 1 are complete and all sorries in `langlands/Langlands/*.lean` are closed. Phase 2 needs Brauer group + Hilbert 90 as prerequisites per the roadmap and hasn't been started. A duplication check against kbuzzard/ClassFieldTheory (the most active external Lean CFT project) found no construction-level overlap with this repo's planned Weil-group route (`W_K^ab ≅ K_v^×`) — that project uses cohomological class formations instead, no Weil group in its code or blueprint. That check was done once and not independently re-verified; worth a fresh look before committing to the route if Phase 2 work starts, since external repos move.
