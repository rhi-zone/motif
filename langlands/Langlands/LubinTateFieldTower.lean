...
`K_1_torsionInK`, to resolve a name collision with the genuine, non-vacuous `K_1` built in
...
same `K` (Gauss's lemma), so it cannot also split completely there. `K_1_torsionInK`, `splits_map_K_1_torsionInK_of_splits`,
...
# The base-field embedding, `K_1_torsionInK = K(F_π[π])`, and the `hsplit`-transport to `K_1_torsionInK`

The natural next step after the torsion-point group `piTorsion hπ hf n` (packaged as an
`AddCommGroup` in `Langlands.LubinTateTorsionGroup`) is the field extension `K_1_torsionInK := K(F_π[π])`
...
file, however, is where `K_1_torsionInK`'s tower structure and the `hsplit`-transport to it are built, and
...
* To adjoin torsion points to a genuine *base field* (rather than the base ring `O`), `K_1_torsionInK` is
...
* The `hsplit`-transport to `K_1_torsionInK` reuses `Langlands.LubinTate.mem_piTorsion_one_of_root_divX_map`
  ...
  requires `[IsFractionRing O K]` throughout rather than only for the transport theorem — `K_1_torsionInK`
...
* **A potential `Algebra O K_1_torsionInK` diamond is avoided by not building that instance at all.**
  Stating the transport requires composing `Algebra O (FractionRing O)` with
  `Algebra (FractionRing O) K_1_torsionInK`, which risks a second, independently-built `Algebra O K_1_torsionInK`
  ...
  K_1_torsionInK` structure `IntermediateField` already supplies for free (`IntermediateField.toAlgebra`) —
  stating the transported conclusion as a polynomial over `Frac(O)` mapped into `K_1_torsionInK` (rather than
  as `Q.map (algebraMap O K_1_torsionInK)`) sidesteps the need for `Algebra O K_1_torsionInK` entirely, so there is no
  second `O`-to-`K_1_torsionInK` map to build or reconcile.
* **A Lean elaboration-order pitfall: pin `O` and `K` explicitly at every `K_1_torsionInK` call site.**
  Even `Field (K_1_torsionInK hπ hf)` alone (`Algebra`/`Field` typeclass search touching `K_1_torsionInK`'s coeSort)
  reproducibly gets "stuck" (`typeclass instance problem is stuck: IsFractionRing O ?m`) whenever
  `K_1_torsionInK` is applied to explicit arguments (`hπ`, `hf`, or even an unrelated `n : ℕ`) without `O`
  and `K` pinned first — this is not about `piTorsion`, `FractionRing.liftAlgebra`, or any custom
  instance in this file: it reproduces even with `K_1_torsionInK`'s body replaced by
  ...
  `K_1_torsionInK hπ hf`, searches with `O`/`K` still as metavariables before the application is fully
  resolved. The fix: supply `O` and `K` explicitly at every call site,
  `K_1_torsionInK (O := O) (K := K) hπ hf` — this is not a workaround for a mathematical diamond (there is
...
* `K_1_torsionInK` : `IntermediateField.adjoin (FractionRing O) (piTorsion hπ hf 1 : Set K)`, the field
  generated over `Frac(O)` by the `π`-torsion points of `F_π` inside `K`.
* `splits_map_K_1_torsionInK_of_splits` : the positive half of the `hsplit`/`K_1_torsionInK` framing question — `Q :=
  P.divX` splitting completely inside `K` (`hsplit`) transports down to `K_1_torsionInK` for free, via
...
/-- **`K_1_torsionInK := K(F_π[π])`**, the field generated over `O`'s field of fractions by the `π`-torsion
points of the Lubin-Tate formal group law `F_π` inside `K`. The first step of the tower
`K_n := K(F_π[π^n])`. -/
def K_1_torsionInK (hπ : Irreducible π) (hf : IsLubinTatePoly π (residueCard O) f) :
...
/-- **`hsplit` transports downward to `K_1_torsionInK`.** Given `Q := P.divX` splits completely inside `K`
(`hsplit`, the hypothesis `card_piTorsion_one_eq_residueCard` needs), it also splits completely
inside `K_1_torsionInK := K(F_π[π])`, viewed (as it naturally is) as a polynomial over `Frac(O)`: `K_1_torsionInK`'s
...
to `K_1_torsionInK` for free once it is already known for `K`; `hsplit` for `K` itself remains a hypothesis
that has to be checked directly. The conclusion is stated over `Frac(O)` rather than `O` directly
(`Q.map (algebraMap O K_1_torsionInK)`) because `IntermediateField.splits_of_splits` only ever needs the
primary `Algebra (FractionRing O) K_1_torsionInK` structure `IntermediateField` already supplies — no
`Algebra O K_1_torsionInK` instance needs to be built (see the module docstring's diamond discussion). `O` and
`K` are supplied explicitly at every `K_1_torsionInK` application (`K_1_torsionInK (O := O) (K := K) hπ hf`) to work
around a confirmed Lean elaboration-order issue: typeclass search on `K_1_torsionInK hπ hf`'s coeSort gets
"stuck" with `O`/`K` as metavariables unless they are pinned before the application elaborates. -/
theorem splits_map_K_1_torsionInK_of_splits
    ...
    ((P.divX.map (algebraMap O (FractionRing O))).map
      (algebraMap (FractionRing O) (K_1_torsionInK (O := O) (K := K) hπ hf))).Splits := by
  classical
  ...
  have hF : ∀ x ∈ (P.divX.map (algebraMap O (FractionRing O))).rootSet K,
      x ∈ K_1_torsionInK (O := O) (K := K) hπ hf := by
    intro x hx
  ...
  exact IntermediateField.splits_of_splits (F := K_1_torsionInK (O := O) (K := K) hπ hf) hsplit' hF

omit [IsUltrametricDist K] [CompleteSpace K] in
/-- **`[Frac(O)(α) : Frac(O)] = q - 1` for any single root `α` of `Q := P.divX`'s image in `K`** —
pure minimal-polynomial theory, no group action or splitting hypothesis needed (unlike `K_1_torsionInK` itself,
