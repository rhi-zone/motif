...
**`K_1` here was renamed to `K_1_old`** (this pass) to resolve a name collision with the genuine,
...
field extension `K_1_old := K(F_π[π])` and the tower `K_n := K(F_π[π^n])`.
...
* The `hsplit`-transport to `K_1_old` (`ROADMAP.md`'s `hsplit`/`K_1_old` framing question, §34 item 2, §35
...
* **A diamond risk anticipated but avoidable.** `ROADMAP.md` anticipated needing a fresh
  `Algebra O K_1_old` instance (composing `Algebra O (FractionRing O)` with `Algebra (FractionRing O)
  K_1_old`) to state the transport, flagging the risk of a second, independently-built instance
...
* **A genuine, reproducible Lean elaboration-order sensitivity, diagnosed and fixed directly.**
  ...
  `K_1_old` was applied to explicit arguments (`hπ`, `hf`, or even an unrelated `n : ℕ`) without `O`
  ...
  `K_1_old`'s body replaced by `IntermediateField.adjoin (FractionRing O) (∅ : Set K)` and an unrelated
  `ℕ` parameter. Lean's elaborator, when checking an instance goal built from an under-elaborated
  application `K_1_old hπ hf`, searches with `O`/`K` still as metavariables before the application is
  fully resolved. The fix, confirmed by the same reproduction: supply `O` and `K` explicitly at
  every call site, `K_1_old (O := O) (K := K) hπ hf` — this is not a workaround for a mathematical
...
`ROADMAP.md`'s `hsplit`/`K_1_old` framing question on its positive side: not "by construction" alone,
