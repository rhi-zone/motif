...
/-- **`piTorsion hπ hf 1 \ {0}` has exactly `residueCard O - 1` elements.** -/
theorem ncard_piTorsion_one_sdiff_zero
  ...
  rw [Set.ncard_sdiff_singleton_of_mem (zero_mem_piTorsion hπ hf 1), ← Nat.card_coe_set_eq,
    card_piTorsion_one_eq_residueCard hOK hπ hπnorm hf hw heq hPdist hPdeg hsplit]

/-- **The main transitivity theorem.** The image in `K` of the `(ResidueField O)ˣ`-orbit of any
nonzero `α ∈ piTorsion hπ hf 1` is exactly `piTorsion hπ hf 1 \ {0}`: a subset of the same finite
cardinality (`ncard_orbit_eq`, `ncard_piTorsion_one_sdiff_zero`) forces equality
(`Set.eq_of_subset_of_ncard_le`). -/
theorem orbit_image_eq_piTorsion_sdiff_zero
    (hPdeg : P.natDegree = residueCard O) (hsplit : (P.divX.map (algebraMap O K)).Splits)
  ...
  have hcard : (piTorsion (K := K) hπ hf 1 \ {0}).ncard ≤
    ...
    rw [Set.ncard_image_of_injective _ Subtype.coe_injective,
      ...
      ncard_piTorsion_one_sdiff_zero hOK hπ hπnorm hf hw heq hPdist hPdeg hsplit]
  have hcardne : Nat.card ↥(piTorsion (K := K) hπ hf 1) ≠ 0 := by
    rw [card_piTorsion_one_eq_residueCard hOK hπ hπnorm hf hw heq hPdist hPdeg hsplit]
    have := two_le_residueCard (O := O); omega
  have hfinite : Set.Finite (piTorsion (K := K) hπ hf 1) :=
    Set.finite_coe_iff.mp (Nat.finite_of_card_ne_zero hcardne)
  exact Set.eq_of_subset_of_ncard_le hsub hcard hfinite.sdiff
