...
two `O_{K_2}` spellings (`integralClosure O_{K_1} baseChangeSplittingField` vs `integralClosure 𝒪[K] baseChangeSplittingField`) as blocked on a
...
`M := baseChangeSplittingField P₂` needs `[Algebra R M]` and `[IsScalarTower R L M]` — i.e. `Algebra ↥(ValuativeRel.
valuation K).valuationSubring (baseChangeSplittingField P₂)` and its compatibility with the existing `Algebra R (K_1 P)`/
`Algebra (K_1 P) (baseChangeSplittingField P₂)` instances. **This instance does not exist in this repo.** `Langlands/
...
`Algebra 𝒪[K] (baseChangeSplittingField P₂)` — mirroring `K_2.instAlgebraO`/`K_2.instAlgebraK` one more time — is new,
straightforward-looking but unbuilt plumbing; checked directly (`grep`, no hits for `Algebra
↥(ValuativeRel.valuation K).valuationSubring (baseChangeSplittingField` anywhere in this repo), not attempted here given
...
`IsAdicComplete`/`IsDiscreteValuationRing`. The `baseChangeSplittingField`-level analogue of that machinery does not exist
