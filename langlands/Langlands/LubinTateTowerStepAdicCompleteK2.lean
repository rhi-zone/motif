...
two `O_{K_2}` spellings (`integralClosure O_{K_1} K_2` vs `integralClosure 𝒪[K] K_2`) as blocked on a
...
(`R := 𝒪[K]`, `L := K_1 P`, `M := K_2 P₂`), establishing the base-relative facts
...
`O_{K_2} := integralClosure O_{K_1} (K_2 P₂)` that `Langlands/LubinTateTowerStep.lean`'s `TowerStep`
...
* `finiteDimensional_K_K_2` / `algebraIsSeparable_K_K_2` : `FiniteDimensional K (K_2 P₂)` (tower law)
  and `Algebra.IsSeparable K (K_2 P₂)` (given `[CharZero K]`), needed to instantiate the base-relative
  general theorems directly at `L := K_2 P₂` (skipping the `K_1` hop), mirroring `K_2.hnorm_K`'s own
...
`M := K_2 P₂` needs `[Algebra R M]` and `[IsScalarTower R L M]` — i.e. `Algebra ↥(ValuativeRel.
valuation K).valuationSubring (K_2 P₂)` and its compatibility with the existing `Algebra R (K_1 P)`/
`Algebra (K_1 P) (K_2 P₂)` instances. **This instance does not exist in this repo.** `Langlands/
...
`Algebra 𝒪[K] (K_2 P₂)` — mirroring `K_2.instAlgebraO`/`K_2.instAlgebraK` one more time — is new,
straightforward-looking but unbuilt plumbing; checked directly (`grep`, no hits for `Algebra
↥(ValuativeRel.valuation K).valuationSubring (K_2` anywhere in this repo), not attempted here given
...
`IsAdicComplete`/`IsDiscreteValuationRing`. The `K_2`-level analogue of that machinery does not exist
...
/-- **`IsScalarTower K (K_1 P) (K_2 P₂)`** for the two-hop composite algebra
`K_2.instAlgebraK`. -/
theorem isScalarTower_K_K_1_K_2 :
    letI := K_2.instAlgebraK (K := K) (P := P) P₂
    IsScalarTower K (K_1 (K := K) P) (K_2 (K' := K_1 (K := K) P) P₂) := by
  letI := K_2.instAlgebraK (K := K) (P := P) P₂
  apply IsScalarTower.of_algebraMap_eq
  intro x
  exact congrFun (K_2.algebraMap_K_eq (K := K) (P := P) P₂) x

/-- **`FiniteDimensional K (K_2 P₂)`**, via the tower law (`Module.Finite.trans`) applied to
`K_1.instFiniteDimensional` and `K_2.instFiniteDimensional`, using `isScalarTower_K_K_1_K_2`. -/
theorem finiteDimensional_K_K_2 :
    letI := K_2.instAlgebraK (K := K) (P := P) P₂
    FiniteDimensional K (K_2 (K' := K_1 (K := K) P) P₂) := by
  letI := K_2.instAlgebraK (K := K) (P := P) P₂
  haveI := isScalarTower_K_K_1_K_2 (K := K) (P := P) P₂
  exact Module.Finite.trans (K_1 (K := K) P) (K_2 (K' := K_1 (K := K) P) P₂)
...
theorem algebraIsSeparable_K_K_2 [CharZero K] :
  ...
  haveI : Algebra.IsIntegral K (K_2 (K' := K_1 (K := K) P) P₂) := Algebra.IsIntegral.of_finite _ _
  exact Algebra.IsSeparable.of_integral K (K_2 (K' := K_1 (K := K) P) P₂)
...
theorem isLocalHom_comp_towerHom_K_2 (hOK : ∀ c : O, ‖algebraMap O K c‖ ≤ 1) {π : O}
    ...
    IsLocalHom ((algebraMap
      ...
      ↥(integralClosure
        ↥(integralClosure ↥(ValuativeRel.valuation K).valuationSubring (K_1 (K := K) P))
        (K_2 (K' := K_1 (K := K) P) P₂))).comp (towerHom (K := K) hOK P)) := by
