...
/-!
# `K`'s norm transports to `K_2` unchanged, at the *base* field (not the outer ring `O`)

`Langlands/LubinTateTowerStepSplittingField.lean`'s `K_2.hOK_transport` transports the *outer ring*
`O`'s norm bound (`hOK : ∀ c : O, ‖algebraMap O K c‖ ≤ 1`) down to `K_2`. This file proves the
...
`NormedField.isAdicComplete_integralClosure_of_finiteDimensional`) directly at `L := K_2`, skipping
the `K_1` hop: **`K`'s own norm (not `O`'s) extends to `K_2` exactly** (an equality, not merely a
...
* `K_2.instAlgebraK` : `Algebra K (K_2 P₂)`, the explicit two-hop composite `K →(algebraMap)→ K_1 P
...
* `K_2.algebraMap_K_eq` : `algebraMap K (K_2 P₂)` really is that two-hop composite, by `rfl`.
* `K_2.hnorm_K` : **`∀ x : K, ‖algebraMap K (K_2 P₂) x‖ = ‖x‖`** — the `hnorm` hypothesis
  `NormedField.isDiscreteValuationRing_valuationSubring_of_finiteDimensional`/
  `NormedField.isAdicComplete_integralClosure_of_finiteDimensional` need, instantiated directly at
  `L := K_2 P₂`. Two applications of `spectralNorm_extends` chained through the two-hop composite:
...
/-- **`Algebra K (K_2 P₂)`**, the explicit two-hop composite `K → K_1 P → K_2 P₂`. Mirrors
...
/-- **`algebraMap K (K_2 P₂)` really is the two-hop composite** — true by definition of
`K_2.instAlgebraK`. -/
theorem K_2.algebraMap_K_eq :
    letI := K_2.instAlgebraK (K := K) (P := P) P₂
    ⇑(algebraMap K (K_2 (K' := K_1 (K := K) P) P₂)) =
...
omit [IsDomain O] [IsDiscreteValuationRing O] [Finite (IsLocalRing.ResidueField O)]
  ...
  [IsDiscreteValuationRing ↥(ValuativeRel.valuation K).valuationSubring] [IsFractionRing O K] in
/-- **`K`'s own norm extends to `K_2` exactly**, for elements of `K` itself — the `hnorm` hypothesis
the base-relative general theorems of `Langlands/LubinTateSplittingFieldDVR.lean`/
`Langlands/LubinTateTowerStepConcrete.lean` need, instantiated directly at `L := K_2 P₂`, skipping the
`K_1` hop. Two applications of `spectralNorm_extends`, chained through `K_2.algebraMap_K_eq`. -/
theorem K_2.hnorm_K :
    letI := K_2.instAlgebraK (K := K) (P := P) P₂
    ∀ x : K, ‖algebraMap K (K_2 (K' := K_1 (K := K) P) P₂) x‖ = ‖x‖ := by
  letI := K_2.instAlgebraK (K := K) (P := P) P₂
  intro x
  have hcoe : algebraMap K (K_2 (K' := K_1 (K := K) P) P₂) x =
      algebraMap (K_1 (K := K) P) (K_2 (K' := K_1 (K := K) P) P₂)
        (algebraMap K (K_1 (K := K) P) x) :=
    congrFun (K_2.algebraMap_K_eq (K := K) (P := P) P₂) x
  rw [K_2.norm_eq_spectralNorm, hcoe, spectralNorm_extends, K_1.norm_eq_spectralNorm,
