...
directly from the existing concrete tower constructions (`K_1 P`, `K_2 (K' := K_1 P) P₂`), not from a
...
* `Level.norm_le_one_of_mem_algebraMap_OL` : the combined bound, `O_L`-membership directly to a norm
  bound in `K_2 (K' := lvl.L) P₂` (one hop further) — folds `§79`'s two separately-composed pieces
  ...
  themselves.
* Two `example`s (`level_K_1`/`level_K_2`, `L := K_1 P` and `L := K_2 (K' := K_1 P) P₂`): the bundled
...
/-- **The combined bound**: `O_L`-membership directly to a norm bound in `K_2 (K' := lvl.L) P₂`
...
theorem Level.norm_le_one_of_mem_algebraMap_OL [Algebra O' lvl.L] (P₂ : O'[X]) :
    letI := lvl.algL
    letI algOL : Algebra lvl.OL (K_2 (K' := lvl.L) P₂) :=
      ((algebraMap lvl.L (K_2 (K' := lvl.L) P₂)).comp
        (algebraMap lvl.OL lvl.L)).toAlgebra
    ∀ (hnormL : ∀ x : K, ‖algebraMap K lvl.L x‖ = ‖x‖) (c : lvl.OL),
      ‖algebraMap lvl.OL (K_2 (K' := lvl.L) P₂) c‖ ≤ 1 := by
  letI := lvl.algL
  letI := lvl.finiteDim
  letI algOL : Algebra lvl.OL (K_2 (K' := lvl.L) P₂) :=
    ((algebraMap lvl.L (K_2 (K' := lvl.L) P₂)).comp
  ...
  have heq : algebraMap lvl.OL (K_2 (K' := lvl.L) P₂) c =
      algebraMap lvl.L (K_2 (K' := lvl.L) P₂) (algebraMap lvl.OL lvl.L c) := rfl
...
/-! ## Concrete check 2: `L := K_2 (K' := K_1 P) P₂` recovers `K_3.norm_le_one_of_mem_O_K2` exactly -/

/-- The concrete `Level` at `L := K_2 (K' := K_1 P) P₂`, from `§78`'s own composite
