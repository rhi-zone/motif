...
report: the `K_1 → K_2` version's conclusion is stated at the **field** level, `algebraMap (K_1 P) K_2
...
`K_2 → K_3` shape (conclusion `algebraMap lvl.OL (K_2 (K' := lvl.L) Pn) gen`); `hα'coe` re-enters only
...
(K_2 (K' := lvl.L) P₂)` instance it introduces *itself*, by `letI`, as the explicit composite
`((algebraMap lvl.L (K_2 …)).comp (algebraMap lvl.OL lvl.L)).toAlgebra`. Feeding it to
...
/-- **The generic `Algebra O (K_2 (K' := lvl.L) Pn)`**, the explicit three-hop composite
`O →(Level.towerHom hOK)→ lvl.OL →(subalgebra inclusion)→ lvl.L →(algebraMap)→ K_2 (K' := lvl.L) Pn`.
...
@[reducible] def Level.instAlgebraO (lvl : Level K) (Pn : lvl.OL[X])
    ...
    Algebra O (K_2 (K' := lvl.L) Pn) :=
  letI := lvl.algL
  ((algebraMap lvl.L (K_2 (K' := lvl.L) Pn)).comp
...
theorem Level.eval_map_towerHom (lvl : Level K) (Pn : lvl.OL[X])
    (hOK : ∀ c : O, ‖algebraMap O K c‖ ≤ 1) {f : O⟦X⟧} (x : K_2 (K' := lvl.L) Pn) :
...
/-- **`algebraMap lvl.OL (K_2 (K' := lvl.L) Pn)` is injective**, generic in `lvl` — the `Level`-level
...
theorem Level.faithfulSMul_OL (lvl : Level K) (Pn : lvl.OL[X]) :
    FaithfulSMul lvl.OL (K_2 (K' := lvl.L) Pn) := by
  rw [faithfulSMul_iff_algebraMap_injective]
  have h1 : Function.Injective (algebraMap lvl.L (K_2 (K' := lvl.L) Pn)) := RingHom.injective _
  have h2 : Function.Injective (algebraMap lvl.OL lvl.L) := Subtype.coe_injective
  intro a b hab
  have ha := IsScalarTower.algebraMap_apply lvl.OL lvl.L (K_2 (K' := lvl.L) Pn) a
  have hb := IsScalarTower.algebraMap_apply lvl.OL lvl.L (K_2 (K' := lvl.L) Pn) b
  rw [ha, hb] at hab
  exact h2 (h1 hab)
...
omit [IsDomain O] [IsDiscreteValuationRing O] [Finite (IsLocalRing.ResidueField O)]
  [Finite (IsLocalRing.ResidueField ↥(ValuativeRel.valuation K).valuationSubring)]
  [Algebra O K] [IsFractionRing O K] in
/-- **`lvl.OL`'s elements, algebra-mapped into `K_2 (K' := lvl.L) Pn`, have norm at most `1`** —
generic in `lvl`, and stated against the `Algebra lvl.OL (K_2 …)` instance **ordinary instance search
...
theorem Level.norm_le_one_of_mem_OL_next (lvl : Level K) (Pn : lvl.OL[X])
    ...
    ‖algebraMap lvl.OL (K_2 (K' := lvl.L) Pn) c‖ ≤ 1 := by
  letI := lvl.algL
  letI := lvl.finiteDim
  have heq : algebraMap lvl.OL (K_2 (K' := lvl.L) Pn) c =
      algebraMap lvl.L (K_2 (K' := lvl.L) Pn) (algebraMap lvl.OL lvl.L c) :=
...
`K_2 (K' := lvl.L) Pn`** — the `Level`-generic `norm_lt_one_of_aeval_P₂_eq_zero`/
...
theorem Level.norm_lt_one_of_root (lvl : Level K) [IsDomain lvl.OL]
  ...
  obtain ⟨hmonic, hdegeq, hc0, -, hweak⟩ :=
    norm_coeff_map_of_isWeaklyEisensteinAt_associated (K := K_2 (K' := lvl.L) Pn)
...
`eval f x` equals the level's generator `α'`, algebra-mapped into `K_2 (K' := lvl.L) Pn` — an equation
...
theorem Level.eval_f_eq_of_root (lvl : Level K) [IsDomain lvl.OL]
    ...
    letI := lvl.instAlgebraO Pn hOK
    NonarchimedeanPowerSeriesEval.eval f x = algebraMap lvl.OL (K_2 (K' := lvl.L) Pn) α' := by
  letI := lvl.algL
  letI := lvl.instAlgebraO Pn hOK
  have hxnorm : ‖x‖ < 1 :=
  ...
  have hb : ∀ (g : PowerSeries lvl.OL) (n : ℕ),
      ‖algebraMap lvl.OL (K_2 (K' := lvl.L) Pn) (PowerSeries.coeff n g)‖ ≤ 1 :=
...
/-- **The next `Level`**: `L := K_2 (K' := lvl.L) Pn`, with `§81`'s `instAlgebraK_of_Level` as its
stored `algL` and finite-dimensionality over `K` by `FiniteDimensional.trans` through `lvl.L` (the
`IsScalarTower K lvl.L (K_2 …)` it needs is exactly `§81`'s `algebraMap_K_eq_of_Level`, fed to
...
@[reducible] def Level.next (lvl : Level K) (Pn : lvl.OL[X]) : Level K where
  ...
  finiteDim := by
    ...
    haveI : IsScalarTower K lvl.L (K_2 (K' := lvl.L) Pn) :=
      IsScalarTower.of_algebraMap_eq (fun x => congrFun (algebraMap_K_eq_of_Level lvl Pn) x)
    exact FiniteDimensional.trans K lvl.L (K_2 (K' := lvl.L) Pn)
...
structure TowerStep (f : O⟦X⟧) (hOK : ∀ c : O, ‖algebraMap O K c‖ ≤ 1) where
  ...
  genNormLt : ‖algebraMap lvl.OL (K_2 (K' := lvl.L) nextPoly) gen‖ < 1
...
theorem TowerStep.eval_f_eq_of_root {f : O⟦X⟧} {hOK : ∀ c : O, ‖algebraMap O K c‖ ≤ 1}
    (st : TowerStep f hOK) {x : K_2 (K' := st.lvl.L) st.nextPoly}
    ...
    NonarchimedeanPowerSeriesEval.eval f x =
      algebraMap st.lvl.OL (K_2 (K' := st.lvl.L) st.nextPoly) st.gen := by
  letI := st.instDomain
  letI := st.instDVR
