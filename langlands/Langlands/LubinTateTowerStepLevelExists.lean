...
## Concrete checks live in a separate file

The `rfl`-checks against the real hand-named theorems (`exists_eisenstein_tower_step_K_2_flat'`,
`exists_piTorsion_translate_of_aeval_P₂_eq_zero`, `adjoin_eq_integralClosure_K_2`,
`finrank_K_3_eq_residueCard`) moved to `Langlands/LubinTateTowerStepLevelExistsCheck.lean`
(`ROADMAP.md §100`): they need to import `LubinTateTowerStepConcreteK2Flat.lean`/
`LubinTateTowerStepK3Degree.lean` directly, and this file sits in the import closure of every
downstream `Level*.lean` file, so keeping those two imports here would put
`LubinTateTowerStepK3Degree.lean` in that closure for no reason but housing checks.
