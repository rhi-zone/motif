...
/-- **`Algebra K (baseChangeSplittingField (K' := lvl.L) P₂)`, generic in `lvl : Level K`** — the `Level`-parametrized
...
@[reducible] def instAlgebraK_of_Level : Algebra K (baseChangeSplittingField (K' := lvl.L) P₂) :=
  letI := lvl.algL
  instAlgebraK_of_algebraL (K := K) (O' := O') (L := lvl.L) P₂

/-- **`algebraMap K (baseChangeSplittingField (K' := lvl.L) P₂)` really is the two-hop composite through `lvl.L`** —
...
theorem algebraMap_K_eq_of_Level :
    ...
    ⇑(algebraMap K (baseChangeSplittingField (K' := lvl.L) P₂)) =
      ⇑(algebraMap lvl.L (baseChangeSplittingField (K' := lvl.L) P₂)) ∘ ⇑(algebraMap K lvl.L) := by
  letI := lvl.algL
  exact algebraMap_K_eq_of_algebraL (K := K) (O' := O') (L := lvl.L) P₂

/-- **The inductive step, `Level`-parametrized**: `K`'s norm extends exactly to
`baseChangeSplittingField (K' := lvl.L) P₂`, given that it already extends exactly to `lvl.L` (`hnormL`) — the
...
theorem hnorm_K_of_Level (hnormL : letI := lvl.algL; ∀ x : K, ‖algebraMap K lvl.L x‖ = ‖x‖) :
    letI := lvl.algL
    letI := instAlgebraK_of_Level lvl P₂
    ∀ x : K, ‖algebraMap K (baseChangeSplittingField (K' := lvl.L) P₂) x‖ = ‖x‖ := by
  letI := lvl.algL
  exact hnorm_K_of_algebraL (K := K) (O' := O') (L := lvl.L) P₂ hnormL

/-- **A norm bound already established in `lvl.L` extends unchanged into
`baseChangeSplittingField (K' := lvl.L) P₂`**, `Level`-parametrized — the restatement of
...
theorem norm_le_one_of_algebraMap_le_one_of_Level {y : lvl.L} (hy : ‖y‖ ≤ 1) :
    ‖algebraMap lvl.L (baseChangeSplittingField (K' := lvl.L) P₂) y‖ ≤ 1 :=
  norm_le_one_of_algebraMap_le_one_of_algebraL (O' := O') (L := lvl.L) P₂ hy

/-- **Injectivity into `lvl.L` extends unchanged into `baseChangeSplittingField (K' := lvl.L) P₂`**, `Level`-parametrized
— the restatement of `injective_algebraMap_comp_of_algebraL`. -/
theorem injective_algebraMap_comp_of_Level {A : Type*} {f : A → lvl.L}
    (hf : Function.Injective f) :
    Function.Injective (fun a => algebraMap lvl.L (baseChangeSplittingField (K' := lvl.L) P₂) (f a)) :=
  injective_algebraMap_comp_of_algebraL (O' := O') (L := lvl.L) P₂ hf

/-! ## Concrete check 1: `level_K_1` (`L := K_1 P`) recovers the real `baseChangeSplittingField`-level facts exactly -/
...
/-! ## Concrete check 2: `level_K_2` (`L := baseChangeSplittingField (K' := K_1 P) P₂`) recovers the real `K_3`-level
facts exactly -/
...
theorem hnorm_K_of_level_K_2 :
    ...
    ∀ x : K, ‖algebraMap K (baseChangeSplittingField (K' := baseChangeSplittingField (K' := K_1 (K := K) P) P₂) P₃) x‖ = ‖x‖ :=
