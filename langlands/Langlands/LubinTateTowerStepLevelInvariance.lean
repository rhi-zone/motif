...
`Set.Mem` unfoldings, dragging 751 `K_2`/`SplittingField` and the `Ideal.Quotient`/`RingCon`
...
`↥(integralClosure lvl.OL (K_2 (K' := lvl.L) Pn))`'s set-membership unfolding, not an instance
...
same family: `Algebra lvl.OL ↥(integralClosure lvl.OL (K_2 (K' := lvl.L) Pn))` — the plain
...
/-- **`algebraMap O (K_2 (K' := lvl.L) Pn)` is injective** — the `Level`-generic
...
theorem Level.faithfulSMul_O_next (lvl : Level K) (Pn : lvl.OL[X])
    ...
    FaithfulSMul O (K_2 (K' := lvl.L) Pn) := by
  letI := lvl.algL
  letI := lvl.instAlgebraO Pn hOK
  rw [faithfulSMul_iff_algebraMap_injective, lvl.algebraMap_O_eq_comp_L Pn hOK]
  exact ((algebraMap lvl.L (K_2 (K' := lvl.L) Pn)).injective.comp
    (algebraMap K lvl.L).injective).comp (IsFractionRing.injective O K)

/-- **`algebraMap lvl.L (K_2 (K' := lvl.L) Pn) ∘ algebraMap O lvl.L = algebraMap O (K_2 (K' :=
...
theorem Level.algebraMap_OSelf_comp_next (lvl : Level K) (Pn : lvl.OL[X])
    ...
    letI := lvl.instAlgebraOSelf (O := O)
    letI := lvl.instAlgebraO Pn hOK
    (algebraMap lvl.L (K_2 (K' := lvl.L) Pn)).comp (algebraMap O lvl.L) =
      algebraMap O (K_2 (K' := lvl.L) Pn) := by
  letI := lvl.algL
  letI := lvl.instAlgebraOSelf (O := O)
  letI := lvl.instAlgebraO Pn hOK
...
/-- **Any `Q : O[X]`'s image over `K_2 (K' := lvl.L) Pn` is its image over `lvl.L`, mapped
...
theorem Level.map_algebraMap_O_next_eq_map (lvl : Level K) (Pn : lvl.OL[X])
    ...
    letI := lvl.instAlgebraOSelf (O := O)
    letI := lvl.instAlgebraO Pn hOK
    Q.map (algebraMap O (K_2 (K' := lvl.L) Pn)) =
      (Q.map (algebraMap O lvl.L)).map (algebraMap lvl.L (K_2 (K' := lvl.L) Pn)) := by
  letI := lvl.algL
  letI := lvl.instAlgebraOSelf (O := O)
  letI := lvl.instAlgebraO Pn hOK
  rw [Polynomial.map_map, lvl.algebraMap_OSelf_comp_next Pn hOK]

/-- **`piTorsion hπ hf 1`, evaluated inside `K_2 (K' := lvl.L) Pn`, is exactly the
`algebraMap lvl.L (K_2 (K' := lvl.L) Pn)`-image of `piTorsion hπ hf 1` evaluated inside `lvl.L`** —
...
Proof: at each of `lvl.L` and `K_2 (K' := lvl.L) Pn`, `piTorsion hπ hf 1 \ {0}` is the root set of
...
`algebraMap lvl.L (K_2 (K' := lvl.L) Pn)` onto the roots of the further-mapped polynomial, which
...
theorem Level.piTorsion_one_next_eq_algebraMap_image (lvl : Level K) (Pn : lvl.OL[X])
    ...
    letI := lvl.instAlgebraOSelf (O := O)
    letI := lvl.instAlgebraO Pn hOK
    (piTorsion (K := K_2 (K' := lvl.L) Pn) hπ hf 1 : Set (K_2 (K' := lvl.L) Pn)) =
      algebraMap lvl.L (K_2 (K' := lvl.L) Pn) '' (piTorsion (K := lvl.L) hπ hf 1) := by
  classical
  letI := lvl.algL
  letI := lvl.instAlgebraOSelf (O := O)
  letI := lvl.instAlgebraO Pn hOK
  ...
  have hrootsmap : (P.divX.map (algebraMap O lvl.L)).roots.map
      ...
      ((P.divX.map (algebraMap O lvl.L)).map
        (algebraMap lvl.L (K_2 (K' := lvl.L) Pn))).roots :=
    hQmonic.roots_map_of_card_eq_natDegree _ hcard
  rw [← lvl.map_algebraMap_O_next_eq_map Pn hOK P.divX] at hrootsmap
  have hfinseteq : Finset.image (algebraMap lvl.L (K_2 (K' := lvl.L) Pn))
      (P.divX.map (algebraMap O lvl.L)).roots.toFinset =
      (P.divX.map (algebraMap O (K_2 (K' := lvl.L) Pn))).roots.toFinset := by
  ...
  have hstep2 := piTorsion_one_sdiff_zero_eq_roots_toFinset (K := K_2 (K' := lvl.L) Pn)
    (lvl.hOK_transport Pn hOK hnormL) hπ (lvl.hπnorm_transport Pn hOK hnormL hπnorm)
    hf hu heq hPdist hPdeg
  have himageeq :
      algebraMap lvl.L (K_2 (K' := lvl.L) Pn) '' (piTorsion (K := lvl.L) hπ hf 1 \ {0}) =
      piTorsion (K := K_2 (K' := lvl.L) Pn) hπ hf 1 \ {0} := by
    rw [hstep1, hstep2, ← Finset.coe_image, hfinseteq]
  have h0 : algebraMap lvl.L (K_2 (K' := lvl.L) Pn) '' (piTorsion (K := lvl.L) hπ hf 1) =
      insert (algebraMap lvl.L (K_2 (K' := lvl.L) Pn) 0)
        (algebraMap lvl.L (K_2 (K' := lvl.L) Pn) ''
...
/-- **`lvl.L⟮β⟯ = ⊤`, for `β` any root of `Pn`'s image in `K_2 (K' := lvl.L) Pn`** — the
`Level`-generic `adjoin_root_eq_top_K_2`/`adjoin_root_eq_top_K_3`.

`K_2 (K' := lvl.L) Pn` is by construction the splitting field of `Pn`'s image over `lvl.L`, hence
...
theorem Level.adjoin_root_eq_top (Pn : lvl.OL[X])
    ...
    letI := lvl.instAlgebraO Pn hOK
    (lvl.L⟮β⟯ : IntermediateField lvl.L (K_2 (K' := lvl.L) Pn)) = ⊤ := by
  letI := lvl.algL
  letI := lvl.instAlgebraOSelf (O := O)
  letI := lvl.instAlgebraO Pn hOK
  have hβnorm : ‖β‖ < 1 :=
    lvl.norm_lt_one_of_root Pn hnormL hα'irr hPndist hassoc hdeg hα'norm hβroot
  have hroots : ((Pn.map (algebraMap lvl.OL lvl.L)).rootSet (K_2 (K' := lvl.L) Pn)) ⊆
      ((lvl.L⟮β⟯ : IntermediateField lvl.L (K_2 (K' := lvl.L) Pn)).toSubalgebra :
        Set (K_2 (K' := lvl.L) Pn)) := by
    intro β' hβ'
  ...
  have hle : (⊤ : Subalgebra lvl.L (K_2 (K' := lvl.L) Pn)) ≤
      (lvl.L⟮β⟯ : IntermediateField lvl.L (K_2 (K' := lvl.L) Pn)).toSubalgebra := by
    rw [← Polynomial.IsSplittingField.adjoin_rootSet (K_2 (K' := lvl.L) Pn)
...
(K_2 (K' := lvl.L) Pn)` — the chosen root really does generate the next field.
...
`Algebra.IsIntegral.of_finite`, `K_2 (K' := lvl.L) Pn` being finite over `lvl.L`. -/
theorem Level.natDegree_minpoly_eq_finrank (Pn : lvl.OL[X])
    ...
    {β : K_2 (K' := lvl.L) Pn}
    (hβroot : Polynomial.aeval β (Pn.map (algebraMap lvl.OL lvl.L)) = 0) :
    (minpoly lvl.L β).natDegree = Module.finrank lvl.L (K_2 (K' := lvl.L) Pn) := by
  letI := lvl.algL
  ...
  rw [← IntermediateField.finrank_top' (F := lvl.L) (E := K_2 (K' := lvl.L) Pn), ← htop,
    IntermediateField.adjoin.finrank hβint]

/-- **`Module.finrank lvl.L (K_2 (K' := lvl.L) Pn) = residueCard O`** — `[K_{n+1} : K_n] = q`,
...
theorem Level.finrank_next_eq_residueCard (Pn : lvl.OL[X])
    ...
    letI := lvl.algL
    Module.finrank lvl.L (K_2 (K' := lvl.L) Pn) = residueCard O := by
  letI := lvl.algL
  have htop := lvl.adjoin_root_eq_top Pn hOK hnormL hπ hπnorm hf hu heq hPdist hPdeg hSplits
    hv heqn hα'irr hPndist hassoc hdeg hα'norm hβroot
  rw [← IntermediateField.finrank_top' (F := lvl.L) (E := K_2 (K' := lvl.L) Pn), ← htop]
