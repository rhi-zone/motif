...
sidesteps entirely the diamond risk a naive proof via `IsScalarTower lvl.OL lvl.L (K_2 (K' :=
lvl.L) Pn)` would run into: that instance is not registered anywhere for an abstract `lvl` (the
`K_2` combinator's own `[Algebra O' K']` hypothesis, instantiated at `O' := lvl.OL`, would need to
...
`Level.algebraMap_O_eq_comp_L` `§83`), never from an `IsScalarTower lvl.OL lvl.L (K_2 (K' :=
...
theorem Level.algebraMap_OSelf_next_eq (lvl : Level K) (Pn : lvl.OL[X])
  ...
  apply RingHom.ext
  intro c
  show algebraMap O (lvl.next Pn).L c = algebraMap O (K_2 (K' := lvl.L) Pn) c
...
theorem Level.splits_next (lvl : Level K) (Pn : lvl.OL[X])
    ...
    letI := lvl.instAlgebraO Pn hOK
    (Q.map (algebraMap O (K_2 (K' := lvl.L) Pn))).Splits := by
  letI := lvl.algL
  letI := lvl.instAlgebraOSelf (O := O)
  letI := lvl.instAlgebraO Pn hOK
  have hmap := hSplits.map (algebraMap lvl.L (K_2 (K' := lvl.L) Pn))
  rw [Polynomial.map_map] at hmap
  rwa [show (algebraMap lvl.L (K_2 (K' := lvl.L) Pn)).comp (algebraMap O lvl.L) =
      algebraMap O (K_2 (K' := lvl.L) Pn) from
