...
integralClosure O_{K_1} (K_2 P₂)` is monogenic over `O_{K_1}`, but residue-field preservation needs
...
* `isLocalRing_integralClosure_of_isDistinguishedAt_root` : a general (non-tower-specific) engine —
  ...
  abstractly (not tied to the Lubin-Tate tower's concrete `K_1`/`K_2` types) precisely to avoid the
...
* `isLocalRing_integralClosure_K_2` : **`IsLocalRing O_{K_2}`**, the instantiation of the above at
  `R := O_{K_1}`, `L := K_2 P₂`, under exactly `adjoin_eq_integralClosure_K_2`'s hypothesis package.
...
`K_1 P`/`K_2 P₂`), so that instantiating it is a single application with no in-proof `set`/rewriting
...
`O_{K_2} := integralClosure O_{K_1} (K_2 P₂)` is a local ring. Instantiation of
`isLocalRing_integralClosure_of_isDistinguishedAt_root` at `R := O_{K_1}`, `L := K_2 P₂`; `hβroot`
