...
`P₂`'s image generating `baseChangeSplittingField` over `K_1 P` (`(K_1 P)⟮β⟯ = ⊤`). This file identifies
...
* `adjoin_eq_integralClosure_K_2` : **`Algebra.adjoin O_{K_1} {β} = integralClosure O_{K_1} (baseChangeSplittingField P₂)`**
...
`[IsLocalRing B]` for `B := integralClosure O_{K_1} (baseChangeSplittingField P₂)` — a hypothesis `IsLocalRing.residueField_
...
/-- **`Algebra.adjoin O_{K_1} {β} = integralClosure O_{K_1} (baseChangeSplittingField P₂)`** — monogenicity of `O_{K_2}`
over `O_{K_1}`, for `β` any root of `P₂`'s image generating `baseChangeSplittingField` over `K_1 P`
...
needed — `Algebra O_{K_1} (baseChangeSplittingField P₂)` and `IsScalarTower O_{K_1} (K_1 P) (baseChangeSplittingField P₂)` resolve by ordinary
...
`Polynomial.aeval_map_algebraMap`, valid since `IsScalarTower O_{K_1} (K_1 P) (baseChangeSplittingField P₂)` holds).
...
(`(minpoly (K_1 P) β).natDegree = Module.finrank (K_1 P) (baseChangeSplittingField P₂)`) from `IntermediateField.adjoin.
