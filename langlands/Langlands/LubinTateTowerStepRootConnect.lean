...
P) nextSplittingField α`" (an equation about the *power-series* `eval`, directly against the original base `O`,
matching what `Langlands.LubinTateRootTranslation`'s generic translation machinery
(`eval_f_FPiEval`, `sub_mem_piTorsion_one_of_eval_f_eq`) needs to be instantiated at `K := nextSplittingField`).

This file closes that connection, in two steps:

* `norm_lt_one_of_aeval_P₂_eq_zero` : a root of `P₂`'s image lies in the open unit ball of `nextSplittingField`.
  Via `Langlands.EisensteinRootNorm`'s ultrametric-only Eisenstein-polygon computation (no
  irreducibility needed, so it applies inside `nextSplittingField`, where `P₂`'s image is *not* irreducible) fed
...
This file does not instantiate `eval_f_FPiEval`/`sub_mem_piTorsion_one_of_eval_f_eq` at `K := nextSplittingField`
...
/-! ## `O_{K_1}`'s image in `nextSplittingField` lands in the closed unit ball -/

omit [IsDomain O] [IsDiscreteValuationRing O] [Finite (ResidueField O)] [IsFractionRing O K] in
/-- **Every element of `O_{K_1}`, algebra-mapped into `nextSplittingField` (via whichever `Algebra O_{K_1} nextSplittingField`
...
theorem K_2.norm_le_one_of_mem_O_K1 {P₂ : (↥(integralClosure ↥(ValuativeRel.valuation K).valuationSubring
    ...
    ‖algebraMap _ (nextSplittingField (K' := K_1 (K := K) P) P₂) c‖ ≤ 1 := by
  have heq : algebraMap ↥(integralClosure ↥(ValuativeRel.valuation K).valuationSubring
        (K_1 (K := K) P)) (nextSplittingField (K' := K_1 (K := K) P) P₂) c =
...
/-- **`algebraMap O_{K_1} nextSplittingField` is injective.** Factors as the injective field embedding `K_1 P →
nextSplittingField` composed with the injective subring inclusion `O_{K_1} → K_1 P` (`Subtype.coe_injective`,
...
instance K_2.instFaithfulSMul_O_K1 {P₂ : (↥(integralClosure ↥(ValuativeRel.valuation K).valuationSubring
  ...
  rw [heq]
  exact (algebraMap (K_1 (K := K) P) (nextSplittingField (K' := K_1 (K := K) P) P₂)).injective.comp
...
/-- **`‖α'‖ < 1` in `nextSplittingField`, discharged from the standing hypothesis set.** `exists_irreducible_
...
< 1`. Transported to `nextSplittingField` by `nextSplittingField.norm_eq_spectralNorm` + `spectralNorm_extends` (the `K_1 P → K_2`
...
and `IsScalarTower.algebraMap_apply` to identify `algebraMap _ nextSplittingField α'` with `algebraMap (K_1 P) nextSplittingField
α` via `hα'coe`. -/
theorem hα'norm_lt_one_of_hram (hOK : ∀ c : O, ‖algebraMap O K c‖ ≤ 1) {π : O}
    ...
    {P₂ : (↥(integralClosure ↥(ValuativeRel.valuation K).valuationSubring
      (K_1 (K := K) P)))[X]} :
    ‖algebraMap _ (nextSplittingField (K' := K_1 (K := K) P) P₂) (α' : _)‖ < 1 := by
  ...
  have hcoe : algebraMap _ (nextSplittingField (K' := K_1 (K := K) P) P₂) (α' : _) =
      algebraMap (K_1 (K := K) P) (nextSplittingField (K' := K_1 (K := K) P) P₂) α := by
    rw [← hα'coe]
    exact IsScalarTower.algebraMap_apply _ (K_1 (K := K) P) _ α'
  rw [hcoe, nextSplittingField.norm_eq_spectralNorm, spectralNorm_extends, K_1.norm_eq_spectralNorm]
  exact hspeclt

omit [IsDomain O] [IsDiscreteValuationRing O] [Finite (ResidueField O)] [IsFractionRing O K] in
/-- **A root of `P₂`'s image (via the `K_1 P` polynomial route) lies in `nextSplittingField`'s open unit ball.**
Via `Langlands.EisensteinRootNorm`'s ultrametric-only Eisenstein-polygon computation — no
irreducibility over a base field is needed, so this applies inside `nextSplittingField` itself, where `P₂`'s image
...
`Polynomial.norm_lt_one_of_isEisensteinShape_of_root` needs. `hα'norm` (`‖α'‖ < 1` in `nextSplittingField`) is
...
theorem norm_lt_one_of_aeval_P₂_eq_zero (hα'irr : Irreducible α')
  ...
  obtain ⟨hmonic, hdegeq, hc0, -, hweak⟩ :=
    norm_coeff_map_of_isWeaklyEisensteinAt_associated (K := nextSplittingField (K' := K_1 (K := K) P) P₂)
...
(the level-`1` generator `P₂`'s constant term is built from), algebra-mapped into `nextSplittingField` — an
...
machinery needs to be instantiated at `K := nextSplittingField`.
...
theorem eval_f_eq_of_aeval_P₂_eq_zero (hOK : ∀ c : O, ‖algebraMap O K c‖ ≤ 1)
  ...
  have hbP₂ : ∀ n, ‖algebraMap _ (nextSplittingField (K' := K_1 (K := K) P) P₂)
  ...
  have hbu₂ : ∀ n, ‖algebraMap _ (nextSplittingField (K' := K_1 (K := K) P) P₂)
  ...
  have hbfmap : ∀ n, ‖algebraMap _ (nextSplittingField (K' := K_1 (K := K) P) P₂)
      (PowerSeries.coeff n (PowerSeries.map (towerHom (K := K) hOK P) f))‖ ≤ 1 :=
    fun n ↦ K_2.norm_le_one_of_mem_O_K1 _
  have hbCα' : ∀ n, ‖algebraMap _ (nextSplittingField (K' := K_1 (K := K) P) P₂)
  ...
  have hmapeq :
      NonarchimedeanPowerSeriesEval.eval (PowerSeries.map (towerHom (K := K) hOK P) f) β =
        algebraMap _ (nextSplittingField (K' := K_1 (K := K) P) P₂) (α' : _) := sub_eq_zero.mp hsub
  rw [eval_map_towerHom (K := K) (P := P) P₂ hOK β] at hmapeq
  have hfinal : algebraMap _ (nextSplittingField (K' := K_1 (K := K) P) P₂) (α' : _) =
      algebraMap (K_1 (K := K) P) (nextSplittingField (K' := K_1 (K := K) P) P₂) α := by
    rw [← hα'coe]
    exact IsScalarTower.algebraMap_apply _ (K_1 (K := K) P) _ α'
  rw [hmapeq, hfinal]
...
nextSplittingField`.** For `β, β'` both roots (via the `K_1 P` polynomial route), there is `t' ∈ piTorsion hπ hf 1`
(evaluated inside `nextSplittingField`) with `β' = F_π(β, t')`. Both `eval f β` and `eval f β'` equal `algebraMap
(K_1 P) nextSplittingField α` by `eval_f_eq_of_aeval_P₂_eq_zero` (applied to each root separately), hence to each
...
`ROADMAP.md`'s tower-step chain, instantiated at `K := nextSplittingField` — no separate "well-definedness converse"
...
omit [IsDomain O] [IsDiscreteValuationRing O] [Finite (ResidueField O)] [IsFractionRing O K] in
/-- **`K_2.instAlgebraO`'s composite collapses to the ordinary two-hop `algebraMap (K_1 P) nextSplittingField ∘
...
This is the single fact everything else in this section reduces to: once `algebraMap O nextSplittingField` is
...
omit [IsDomain O] [IsDiscreteValuationRing O] [Finite (ResidueField O)] [IsFractionRing O K] in
/-- **`K`'s strict uniformizer bound (`hπnorm`) transports down to `nextSplittingField` unchanged.** The `nextSplittingField`
...
theorem K_2.hπnorm_transport (hOK : ∀ c : O, ‖algebraMap O K c‖ ≤ 1) {π : O}
    ...
    letI := K_2.instAlgebraO (K := K) (P := P) P₂ hOK
    ‖algebraMap O (nextSplittingField (K' := K_1 (K := K) P) P₂) π‖ < 1 := by
  letI := K_2.instAlgebraO (K := K) (P := P) P₂ hOK
  have hcoe : algebraMap O (nextSplittingField (K' := K_1 (K := K) P) P₂) π =
      algebraMap (K_1 (K := K) P) (nextSplittingField (K' := K_1 (K := K) P) P₂)
...
/-- **`algebraMap O nextSplittingField` is injective.** The composite of `algebraMap O (K_1 P)` (injective,
`K_1.instFaithfulSMul`) and `algebraMap (K_1 P) nextSplittingField` (injective, any ring hom out of a field),
identified via `K_2.algebraMap_O_eq_comp_K_1`. -/
theorem K_2.instFaithfulSMul_O (hOK : ∀ c : O, ‖algebraMap O K c‖ ≤ 1) :
    letI := K_2.instAlgebraO (K := K) (P := P) P₂ hOK
    FaithfulSMul O (nextSplittingField (K' := K_1 (K := K) P) P₂) := by
  letI := K_2.instAlgebraO (K := K) (P := P) P₂ hOK
  rw [faithfulSMul_iff_algebraMap_injective]
...
omit [IsDomain O] [IsDiscreteValuationRing O] [Finite (ResidueField O)] [IsFractionRing O K] in
/-- **`Q := P.divX`'s image over `nextSplittingField` (via `K_2.instAlgebraO`) is `Q`'s image over `K_1 P`, further
mapped along `algebraMap (K_1 P) nextSplittingField`.** `Polynomial.map_map` plus
...
`Polynomial.Monic.roots_map_of_card_eq_natDegree` supplies at `K_1 P` up to `nextSplittingField`. -/
theorem divX_map_algebraMap_O_K_2_eq_map (hOK : ∀ c : O, ‖algebraMap O K c‖ ≤ 1) :
    letI := K_2.instAlgebraO (K := K) (P := P) P₂ hOK
    P.divX.map (algebraMap O (nextSplittingField (K' := K_1 (K := K) P) P₂)) =
      (P.divX.map (algebraMap O (K_1 (K := K) P))).map
        (algebraMap (K_1 (K := K) P) (nextSplittingField (K' := K_1 (K := K) P) P₂)) := by
  letI := K_2.instAlgebraO (K := K) (P := P) P₂ hOK
  have hcomp : (algebraMap (K_1 (K := K) P) (nextSplittingField (K' := K_1 (K := K) P) P₂)).comp
      (algebraMap O (K_1 (K := K) P)) = algebraMap O (nextSplittingField (K' := K_1 (K := K) P) P₂) :=
    RingHom.ext fun c =>
...
/-- **`piTorsion hπ hf 1`, evaluated inside `nextSplittingField`, is exactly the `algebraMap (K_1 P) nextSplittingField`-image of
`piTorsion hπ hf 1` evaluated inside `K_1 P`.** The level-`1` `π`-torsion does not grow when passing
from `K_1 P` to `nextSplittingField` — `ROADMAP.md` item 6's core missing fact.

Proof: at each of `K := K_1 P` and `K := nextSplittingField`, `piTorsion hπ hf 1 \ {0}` is exactly the root set of
...
the roots multiset along `algebraMap (K_1 P) nextSplittingField` onto the roots of the further-mapped polynomial —
which `divX_map_algebraMap_O_K_2_eq_map` identifies with `Q`'s image over `nextSplittingField` itself. Converting the
...
theorem piTorsion_one_K_2_eq_algebraMap_image (hOK : ∀ c : O, ‖algebraMap O K c‖ ≤ 1)
    ...
    (hPdeg : P.natDegree = residueCard O) :
    letI := K_2.instAlgebraO (K := K) (P := P) P₂ hOK
    (piTorsion (K := nextSplittingField (K' := K_1 (K := K) P) P₂) hπ hf 1 : Set (nextSplittingField (K' := K_1 (K := K) P) P₂)) =
      algebraMap (K_1 (K := K) P) (nextSplittingField (K' := K_1 (K := K) P) P₂) ''
  ...
  have hrootsmap : (P.divX.map (algebraMap O (K_1 (K := K) P))).roots.map
      (algebraMap (K_1 (K := K) P) (nextSplittingField (K' := K_1 (K := K) P) P₂)) =
      ((P.divX.map (algebraMap O (K_1 (K := K) P))).map
        (algebraMap (K_1 (K := K) P) (nextSplittingField (K' := K_1 (K := K) P) P₂))).roots :=
    hQmonic.roots_map_of_card_eq_natDegree _ hcard
  rw [← divX_map_algebraMap_O_K_2_eq_map (K := K) (P := P) (P₂ := P₂) hOK] at hrootsmap
  have hfinseteq : Finset.image (algebraMap (K_1 (K := K) P) (nextSplittingField (K' := K_1 (K := K) P) P₂))
      (P.divX.map (algebraMap O (K_1 (K := K) P))).roots.toFinset =
      (P.divX.map (algebraMap O (nextSplittingField (K' := K_1 (K := K) P) P₂))).roots.toFinset := by
  ...
  have hstep2 := piTorsion_one_sdiff_zero_eq_roots_toFinset
    (K := nextSplittingField (K' := K_1 (K := K) P) P₂) (K_2.hOK_transport (K := K) (P := P) P₂ hOK) hπ
    (K_2.hπnorm_transport (K := K) (P := P) (P₂ := P₂) hOK hπnorm) hf hu heq hPdist hPdeg
  have himageeq :
      algebraMap (K_1 (K := K) P) (nextSplittingField (K' := K_1 (K := K) P) P₂) ''
        (piTorsion (K := K_1 (K := K) P) hπ hf 1 \ {0}) =
      piTorsion (K := nextSplittingField (K' := K_1 (K := K) P) P₂) hπ hf 1 \ {0} := by
    rw [hstep1, hstep2, ← Finset.coe_image, hfinseteq]
  have h0 : algebraMap (K_1 (K := K) P) (nextSplittingField (K' := K_1 (K := K) P) P₂) ''
      (piTorsion (K := K_1 (K := K) P) hπ hf 1) =
      insert (algebraMap (K_1 (K := K) P) (nextSplittingField (K' := K_1 (K := K) P) P₂) 0)
        (algebraMap (K_1 (K := K) P) (nextSplittingField (K' := K_1 (K := K) P) P₂) ''
