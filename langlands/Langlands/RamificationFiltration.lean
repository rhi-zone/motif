...
**The actual wall is the *kernel* direction, not the homomorphism property.** The task is to show
the kernel of the restriction of this map to `ramificationGroup K A i` is *exactly*
`ramificationGroup K A (i + 1)`. The `⊇` inclusion is immediate (test the `∀ x` defining property
of `G_{i+1}` at `x = π`). The `⊆` inclusion needs the converse: `σ(π) - π ∈ 𝔪_A^{i+2}` (a condition
tested only at the single point `π`) must imply `σ(x) - x ∈ 𝔪_A^{i+2}` for *every* `x : A` (Serre,
*Local Fields*, Ch. IV, the equivalence between his `i_G(σ) = v_L(σ(π_L) - π_L)`-based definition
and the "acts trivially on `O_L / 𝔪_L^{i+1}`" definition used by this file's `ramificationGroup`).
Classically this step uses that the ring of integers of a totally ramified extension of a complete
discretely-valued field is monogenic, generated by any uniformizer (`O_L = O_{L_0}[π_L]`, `L_0` the
maximal unramified subextension) — an Eisenstein-polynomial argument that needs completeness. This
session checked (`grep -rli "monogenic\|PowerBasis"` over
`Mathlib/RingTheory/Valuation/` and `Mathlib/RingTheory/DiscreteValuationRing/`, both empty) that
**no monogenicity-of-DVR-extension theorem exists in Mathlib** to supply this step, and it is not a
narrow hypothesis to add on top of this file's `A : ValuationSubring L` generality — stating it
directly (e.g. "`∃ x, A = Algebra.adjoin (fixed field) {x}`, with `x` related to `π`") would amount
to assuming most of the content of the theorem being proved, not a legitimate infrastructure gap to
paper over. Per this project's stop-on-genuine-wall discipline, this is left undone rather than
forced with an extra ad hoc hypothesis or a `sorry`.

Consequently the classical "eventually trivial" finiteness fact (the filtration hits `⊥` past some
`N`, under a finiteness hypothesis such as finiteness of the residue field) is also not attempted,
since its standard proof is built on top of these associated-graded embeddings. See `ROADMAP.md`,
Phase 2b, for the tracked gap.
-/
