...
/-- If `x : L` is a root of the monic lift `f` of `minpoly k β₀`, then `f.map (algebraMap R K) =
minpoly K x` and `K⟮x⟯ / K` is separable. Extracted from
`exists_isDiscreteValuationRing_integralClosure_residueField_equiv`, which supplies `x` via
`exists_ringHom_adjoinRoot_map_of_isAlgClosed`: unlike that existence statement, this lemma needs no
hypothesis on `L` beyond the scalar tower `R → K → L`, since it only uses the given root `x`, not
algebraic closedness. `f.map (algebraMap R K)` is irreducible (`irreducible_map_lift_minpoly`) and
monic, so `minpoly.eq_of_irreducible` identifies it with `minpoly K x` up to a unit scalar, resolved
by monicity of both sides; separability follows from `separable_map_lift_minpoly` transported along
that identification. -/
theorem HenselianLocalRing.map_eq_minpoly_and_isSeparable_of_aeval_eq_zero
    {R : Type*} [CommRing R] [IsDomain R] [IsIntegrallyClosed R] [HenselianLocalRing R]
    ...
    (hfmap : f.map (algebraMap R (IsLocalRing.ResidueField R)) =
      minpoly (IsLocalRing.ResidueField R) β₀)
    {x : L} (hfx : Polynomial.aeval x f = 0) :
    f.map (algebraMap R K) = minpoly K x ∧
      Algebra.IsSeparable K ↥(IntermediateField.adjoin K {x}) := by
  ...
  have hpx : Polynomial.aeval x (f.map (algebraMap R K)) = 0 := by
    rw [Polynomial.aeval_def, Polynomial.eval₂_map, ← IsScalarTower.algebraMap_eq R K L,
...
/-- Given a root `x : L` of `f` (integral over `R`, via `hfx`) with `f.map (algebraMap R K) =
minpoly K x` and `K⟮x⟯ / K` separable, the integral closure `C` of `R` in `K⟮x⟯` is monogenic over
`R`, generated by the image `xC` of `x`, which is itself a root of `f`. Extracted from
`exists_isDiscreteValuationRing_integralClosure_residueField_equiv`: this is exactly its
monogenicity conjunct, `∃ β : C, Algebra.adjoin R {β} = ⊤`, together with the fact (`hxCroot`) that
`xC` is a root of `f` in `C`, needed by the caller to build `AdjoinRoot f →+* C`. Proved via
`Algebra.adjoin_eq_top_of_isUnit_aeval_derivative_minpoly` (`Langlands.MonogenicMaximalOrder`), whose
`hunit` hypothesis is transferred from the "discriminant is a unit" fact `isUnit_resultant_lift_minpoly`
by `Polynomial.isUnit_aeval_derivative_of_isUnit_resultant`. The third conjunct records that `xC`
lies over `x` (via the intermediate field `K' := K⟮x⟯`), needed by the caller to identify `xC` as
*the* witness of monogenicity that a `y : C` over `x` must equal, not just some witness. -/
theorem HenselianLocalRing.exists_adjoin_eq_top_integralClosure_of_root
    {R : Type*} [CommRing R] [IsDomain R] [IsIntegrallyClosed R] [IsDedekindDomain R]
    [HenselianLocalRing R] [Finite (IsLocalRing.ResidueField R)]
    {K : Type*} [Field K] [Algebra R K] [IsFractionRing R K]
    {L : Type*} [Field L] [Algebra K L] [Algebra R L] [IsScalarTower R K L]
    {l : Type*} [Field l] [Algebra (IsLocalRing.ResidueField R) l] {β₀ : l}
    (hβ₀ : IsIntegral (IsLocalRing.ResidueField R) β₀)
    ...
    (hfmap : f.map (algebraMap R (IsLocalRing.ResidueField R)) =
      minpoly (IsLocalRing.ResidueField R) β₀)
    {x : L} (hfx : Polynomial.aeval x f = 0)
    (hminpoly : f.map (algebraMap R K) = minpoly K x)
    (hK'sep : Algebra.IsSeparable K ↥(IntermediateField.adjoin K {x})) :
    ∃ xC : integralClosure R (IntermediateField.adjoin K {x}),
      Algebra.adjoin R ({xC} : Set (integralClosure R (IntermediateField.adjoin K {x}))) = ⊤ ∧
        Polynomial.aeval xC f = 0 ∧
        algebraMap ↥(IntermediateField.adjoin K {x}) L
            (algebraMap (integralClosure R (IntermediateField.adjoin K {x}))
              ↥(IntermediateField.adjoin K {x}) xC) = x := by
  have hxint : IsIntegral R x := ⟨f, hfmonic, hfx⟩
  have hxK : IsIntegral K x := hxint.tower_top
  ...
  haveI : Module.IsTorsionFree R C :=
    Module.IsTorsionFree.of_smul_eq_zero fun r c hrc => by
      ...
      · right; exact h
  have hxCL : algebraMap K' L (algebraMap C K' xC) = x := by
    show algebraMap K' L (x' : K') = x
    exact hxL
  exact ⟨xC, Algebra.adjoin_eq_top_of_isUnit_aeval_derivative_minpoly R K K' xC hxadjoinK hxCunit,
    hxCroot, hxCL⟩
...
theorem HenselianLocalRing.exists_isDiscreteValuationRing_integralClosure_residueField_equiv
  ...
  have hfx : Polynomial.aeval x f = 0 := by
    ...
    exact this
  have hxint : IsIntegral R x := ⟨f, hfmonic, hfx⟩
  -- Stage A+B (existence of the root `x` and its minimal-polynomial / separability facts) is
  -- `map_eq_minpoly_and_isSeparable_of_aeval_eq_zero`, extracted as a standalone lemma: it needs
  -- nothing beyond `x` and its being a root of `f`, not the algebraic closedness of `L` used above
  -- only to produce `x` in the first place.
  obtain ⟨hminpoly, hK'sep⟩ :=
    HenselianLocalRing.map_eq_minpoly_and_isSeparable_of_aeval_eq_zero (K := K) hβ₀ hfmonic hfmap
      hfx
  ...
  have hRC_inj : Function.Injective (algebraMap R C) := by
    ...
    have h := congrArg (algebraMap C K') hab
    rwa [← IsScalarTower.algebraMap_apply R C K', ← IsScalarTower.algebraMap_apply R C K'] at h
  -- Stage C (monogenicity of `C := integralClosure R K'`, generated by the image `xC` of `x`) is
  -- `exists_adjoin_eq_top_integralClosure_of_root`, extracted as a standalone lemma: it is exactly
  -- the theorem's own monogenicity conjunct, self-contained given `x`, `hminpoly` and `hK'sep`.
  obtain ⟨xC, hCtop, hxCroot, hxCL⟩ :=
    HenselianLocalRing.exists_adjoin_eq_top_integralClosure_of_root hβ₀ hfmonic hfmap hfx hminpoly
      hK'sep
  ...
  have hcompeq : (algebraMap C L).comp ψ = ψ₀ := by
    ...
    · rw [RingHom.comp_apply]
      ...
      show algebraMap K' L (algebraMap C K' xC) = x
      exact hxCL
