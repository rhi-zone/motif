...
2. **Injectivity**, which is unaffected by the universe question and is the deeper content: it
   ...
   diagram `n ↦ ℤ ⧸ nℤ` defining `Zhat`. Two general-purpose (non-`Langlands`-specific) pieces of
   this are now proved from scratch and committed: `IsCyclic.existsUnique_subgroup_card_eq`
   (`Langlands.CyclicSubgroups`, pure group theory -- a finite cyclic group has a unique subgroup
   of each order dividing its own) and `IsGalois.existsUnique_intermediateField_finrank_eq`
   (`Langlands.CyclicGaloisSubfields` -- its field-theoretic translation through the Galois
   correspondence: a *finite* cyclic Galois extension `E/F` has a unique subfield of each degree
   dividing `[E:F]`). Specialized here: `isCyclic_gal_of_finite_normal` (an `instance`) shows
   `Gal(M/𝓀[K])` is cyclic (generated by Frobenius restricted) for *every* finite Galois `M ≤ kbar`
   -- via `exists_frobenius_pow_eq` applied directly to an arbitrary `h : Gal(M/𝓀[K])`, no lifting
   to `kbar` needed -- and `exists_intermediateField_finrank_eq` gives *existence* (via embedding
   `FiniteField.Extension 𝓀[K] (residueCharP K) n` into `kbar` through `IsAlgClosed.lift`, since
   `kbar` is algebraically closed) of a degree-`n` subfield of `kbar` for every `n`. What is still
   missing: *uniqueness* of that degree-`n` subfield *globally* across all of `kbar` (as opposed to
   within one fixed finite Galois subextension, which `IsGalois.existsUnique_intermediateField_finrank_eq`
   already gives) -- comparing two arbitrary degree-`n` subfields `M₁ M₂ ≤ kbar` needs relating them
   to a common finite Galois subextension containing both (e.g. `M₁ ⊔ M₂` in the
   `Lattice (FiniteGaloisIntermediateField 𝓀[K] kbar)` structure) and viewing `M₁`, `M₂` as
   intermediate fields *of that smaller ambient field* rather than of `kbar`, for which no
   ready-made Mathlib "restrict an intermediate field to a smaller ambient intermediate field"
   operation was located; and then, given the resulting order-isomorphism between finite Galois
   subextensions of `kbar/𝓀[K]` and `(ℕ, ∣)`, the final assembly into a map to/from `Zhat` (whose
   defining diagram is indexed by `FiniteIndexNormalSubgroup (Multiplicative ℤ)`, itself not yet
   related to `(ℕ, ∣)` here) -- either via the universe-obstructed categorical route in item 1
   above, or a bare-hands construction of `Zhat`'s limit elements directly, neither attempted. -/
