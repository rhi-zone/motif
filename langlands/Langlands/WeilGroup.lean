...
* `exists_residueGaloisGroup_equiv_Zhat` : the identification `Gal(𝓀[K̄]/𝓀[K]) ≅ ℤ̂` via Frobenius,
  ...
  genuinely separate gaps (see the docstring of `exists_residueGaloisGroup_equiv_Zhat` for details):
  (1) a **universe obstruction** discovered while trying to apply
  `ProfiniteGrp.ProfiniteCompletion.lift` to `frobeniusZpowersHom` bundled via
  `InfiniteGalois.profiniteGalGrp` -- `Zhat` is hardwired to universe `0` (it completes
  `Multiplicative ℤ : Type`) while `kbar` lives in `K`'s own (`variable`-bound, arbitrary) universe,
  and Mathlib's categorical profinite-completion API requires source and target in the *same*
  universe, so a `ULift` bridge (not attempted here) is needed for general `K`; density above would
  then give surjectivity for free (compact image, closed since `Gal(kbar/𝓀[K])` is Hausdorff, dense,
  hence everything); and (2) **injectivity**, the deeper content, needing existence *and uniqueness*
  of a degree-`n` subfield of `kbar` over `𝓀[K]` for every `n` (no such classification lemma found in
  Mathlib) to match the open normal subgroups of `Gal(kbar/𝓀[K])` against the finite-index-normal-
  subgroup diagram `n ↦ ℤ ⧸ nℤ` defining `Zhat`.
...
2. **Injectivity**, which is unaffected by the universe question and is the deeper content: it
   ...
   diagram `n ↦ ℤ ⧸ nℤ` defining `Zhat` -- concretely, existence *and uniqueness* of a degree-`n`
   subfield of `kbar` over `𝓀[K]` for every `n`, which is not yet available in Mathlib in a
   reusable form (no lemma found classifying subfields of a fixed algebraic closure of a finite
   field by degree). -/
